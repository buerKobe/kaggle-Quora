...
dump_svmlight_file(X,y,path+"X_tfidf.svm") #以svmlight文件格式转储数据集
del X
dump_svmlight_file(X_t,np.zeros(X_t.shape[0]),path+"X_t_tfidf.svm")
del X_t

def oversample(X_ot,y,p=0.165):  #对不均衡数据进行重抽样
    pos_ot = X_ot[y==1]
    neg_ot = X_ot[y==0]
    #p = 0.165
...
X_train,y_train = shuffle(X_train,y_train,random_state=seed) #随机排序
