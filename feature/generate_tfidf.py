...
vect_orig = TfidfVectorizer(max_features=max_features,ngram_range=ngram_range, min_df=min_df) #TF-IDF值
corpus = [] #存放question
for f in feats:
...
print vect_orig
for f in feats:
    tfidfs = vect_orig.transform(data_all[f].values.tolist()) #转化成向量空间模型
...
vect_orig = TfidfVectorizer(max_features=max_features,ngram_range=ngram_range, min_df=min_df) #提取文本特征值
corpus = []
for f in feats:
    data_all[f] = data_all[f].astype(str)
    corpus+=data_all[f].values.tolist()
