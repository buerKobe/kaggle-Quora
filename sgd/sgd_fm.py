...
def mse_loss_function(y,p): #均方根误差函数
    return (y - p)**2

def mae_loss_function(y,p): #均方误差函数
    y = exp(y)
    p = exp(p)
    return abs(y - p)

def log_loss_function(y,p): #对数损失函数
    return -(y*log(p)+(1-y)*log(1-p))

def exponential_loss_function(y,p): #boosting指数误差
...
class SGD(object): #定义SGD-随机梯度下降模型
...
#   dropout=0.9,lr=0.0005,nb_epoch=29
path = "./input/"
...
#	dp=str(n)
#	print "************"+dp+"**************"
sgd = SGD(lr=0.0005,adam=True,dropout=0.8,l2=0.00,l2_fm=0.00,task='c',n_components=1,nb_epoch=29,interaction=True,no_norm=False)
...
# dtest = xgb.DMatrix(x_test)  #数组对象转化为DMatrix格式 才能供xgb训练
# # print dtrain #output:<xgboost.core.DMatrix object at 0x0000000008D979E8>
...
#     'silent': 1,
#     'subsample': 0.6, #调参0.6
#     'learning_rate': 0.01, #调参0.01
#     'objective': 'reg:linear',
#     'max_depth': 1,  #调参1
#     'num_parallel_tree': 1,
#     'min_child_weight': 1,
#     'eval_metric': 'error', #回归用rmse(均方根误差=标准差),分类用error(错误率)
# }

# #xgboost交叉检验，每次迭代输出均值和方差先将数据集先切分成nfold份，然后每次迭代的时候，都会测试nfold次，
# #(nfold-1)做训练集，nfold做测试集，每次迭代给出评测指标的均值和方差。
...
# print('Ensemble-CV: {0}+{1}'.format(cv_mean, cv_std)) #xgb错误率的均值和方差

# gbdt = xgb.train(xgb_params, dtrain, best_nrounds)
# submission = pd.read_csv(SUBMISSION_FILE)
# submission.iloc[:, 1] = gbdt.predict(dtest) #预测值放在第2列后面作为第3列
# # print submission.shape
# saleprice = np.exp(submission['is_duplicate'])-1 #指数幂还原预测值
# submission['is_duplicate'] = saleprice
